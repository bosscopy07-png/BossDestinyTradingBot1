...
try:
    if se_start_auto_scanner:
        try:
            ...
            se_start_auto_scanner(
                pairs=PAIRS,
                interval=SIGNAL_INTERVAL_DEFAULT,
                exchanges=os.getenv("SCAN_EXCHANGES", "binance,bybit,kucoin,okx").split(","),
                min_confidence=AUTO_CONFIDENCE_THRESHOLD,
                poll_seconds=int(os.getenv("SCAN_POLL_SEC", 300)),
            )
