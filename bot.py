...
def fetch_klines_df(symbol="BTCUSDT", interval="1h", limit=200):
    ...
    try:
        ...
        errors.append(str(e)); log_info(f"Binance error {symbol}/{interval}: {e}")
    try:
        return fetch_klines_bybit(symbol, interval, limit)
    except Exception as e:
        errors.append(str(e)); log_info(f"Bybit error {symbol}/{interval}: {e}")
    try:
        return fetch_klines_coingecko(symbol, interval, limit)
    except Exception as e:
        errors.append(str(e)); log_info(f"CoinGecko error {symbol}/{interval}: {e}")
...
def callback_handler(call):
    try:
        ...
        if data == "trending_pairs":
            ...
            if not trends:
                bot.send_message(cid, "No trending data.")
                return
            txt = "📈 Trending Pairs:\n"
            for t in trends:
                txt += f"- {t['pair']}: ATR={t['atr']:.6f}, Vol={int(t['vol'])}\n"
            send_brand_then_text(cid, txt)
            return

        if data == "market_news":
            updates = fetch_market_news(5)
            if not updates:
                bot.send_message(cid, "No news available.")
                return
            txt = "📰 Market News:\n"
            for u in updates:
                txt += f"- {u['title']} ({u.get('category','')})\n"
            send_brand_then_text(cid, txt)
            return
        ...
        if data.startswith("ai_sig_"):
            ...
            if not rec:
                bot.answer_callback_query(call.id, "Signal not found.")
                return
            prompt = f"Provide a CLEAR buy-or-sell decision for this signal, short rationale, recommended leverage & risk controls:\n{json.dumps(rec['signal'], indent=2)}"
            out = ask_ai(prompt)
            bot.send_message(cid, f"🤖 AI Analysis:\n{out}")
            bot.answer_callback_query(call.id, "AI sent.")
            return

        if data == "refresh_bot":
            bot.answer_callback_query(call.id, "Refreshed ✅")
            send_brand_then_text(cid, "Bot refreshed and alive ✅", reply_markup=main_menu())
            return

        if data == "history":
            recent = d.get("signals", [])[-40:]
            if not recent:
                bot.send_message(cid, "No history.")
                return
            txt = "Recent signals:\n"
...
def link_handler(message):
    try:
        ...
        if message.from_user.id == ADMIN_ID:
            ...
            save_data(d)
        bot.reply_to(message, f"Linked {sig_id} as {tag}. Admin must confirm to update balance.")
    except Exception as e:
